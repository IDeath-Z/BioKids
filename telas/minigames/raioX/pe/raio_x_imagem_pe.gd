...
@onready var area_pe = $AreaPe
@onready var animacao_pe = $AreaPe/AnimacaoPe

@onready var info_lado_esquerdo = $InfoPeLadoEsquerdo
@onready var animacao_info_lado_esquerdo = $InfoPeLadoEsquerdo/AnimacaoLadoEsquerdo
@onready var info_lado_direito = $InfoPeLadoDireito
@onready var animacao_info_lado_direito = $InfoPeLadoDireito/AnimacaoLadoDireito
...
func _on_botao_proximo_pressed() -> void:
	...
	info_lado_esquerdo.visible = false
	animacao_pe.play("lado_direito") # Cai na linha 83
	
func _on_pop_up_dicas_botao_audio_pressed() -> void:
	# Colocar o audio aqui quando tiver
	pass

func _on_animacao_pe_animation_started(anim_name: StringName) -> void:
	if anim_name == "fade":
		await get_tree().create_timer(0.1).timeout
		area_pe.visible = true
		texto.visible = true
		botoes_s_n.visible = true

func _on_animacao_pe_animation_finished(anim_name: StringName) -> void:
...
func _on_animacao_lado_esquerdo_animation_finished(anim_name: StringName) -> void:
	if anim_name == "fade":
		if estado_atual == Estado.INICIAL:
			animacao_pe.play_backwards("lado_esquerdo")
			botao_sim.text = "Sim"
			botao_nao.text = "Não"
