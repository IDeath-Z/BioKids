...
var tipos_possiveis = ["azul", "verde", "vermelha", "rosa", "verde maligna"]
...
@onready var contador_label = $UI/ContadorLabel  # Referência ao Label do contador
...
func _ready():
	# Configura o fundo dos botões
	var style = StyleBoxFlat.new()
	style.bg_color = Color(0, 0.5, 0, 1)  # Verde escuro
	$UI/JogarNovamente.add_theme_stylebox_override("normal", style)
	$UI/VoltarMenu.add_theme_stylebox_override("normal", style)
	style = StyleBoxFlat.new()
	style.bg_color = Color(0, 0.7, 0, 1)  # Verde mais claro ao passar o mouse
	$UI/JogarNovamente.add_theme_stylebox_override("hover", style)
	$UI/VoltarMenu.add_theme_stylebox_override("hover", style)
	contador_label.text = "Pontos: " + str(pontos)  # Inicializa o contador
	proxima_fase()
...
func _on_bact_clicada(tipo: Tipo):
	if tipo == missao_atual:
		acertos += 1
		pontos += 1  # Incrementa o contador de pontos
		mensagem_label.text = mensagens_certas[randi() % mensagens_certas.size()]
		check_icon.visible = true
		x_icon.visible = false
		contador_label.text = "Pontos: " + str(pontos)  # Atualiza o Label com o contador
	else:
...
func _on_jogar_novamente_pressed():
	fase = 0
	pontos = 0  # Reseta o contador ao reiniciar
	fato_label.text = ""
	$UI/JogarNovamente.visible = false
	$UI/VoltarMenu.visible = false
	ursinho.texture = load("res://assets/images/Ursinho normal.png")
	contador_label.text = "Pontos: " + str(pontos)  # Atualiza o Label para 0
	proxima_fase()
